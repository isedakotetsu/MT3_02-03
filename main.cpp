...
const char kWindowTitle[] = "LE2C_02_イセダ_コテツ";
...
bool IsCollision(const Segment& segment, const Plane& plane) {
	// 始点と終点から法線方向への距離
	float startDistance = Dot(plane.normal, segment.start) - plane.distance;
	float endDistance = Dot(plane.normal, segment.end) - plane.distance;

	// 距離の符号が異なれば交差している（0を挟んでいる）
	return (startDistance * endDistance <= 0.0f);
}
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		bool hit = IsCollision(segment, plane);
		...
		// 当たり判定と色変更
		uint32_t segmentColor = hit ? RED : BLUE;
